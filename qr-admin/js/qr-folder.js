...
customElements.define("qr-dialog", class extends HTMLElement {
  constructor() {
    ...
    this.addEventListener("transitionend", () => {
      if (this.className === "off") this.className = "";
    });
  }
  title = qr.createElement("p");
  message = qr.createElement("p");
  input = qr.createElement("input");
  ...
  static setUp() {
    ...
    function handleResponse(message) {
      switch (message.action) {
        ...
        case "updateDirs": {
          for (let dir of message.changedDirs) {
            let dirpath = dir.dirpath;
            deepIntoFile(dirpath, !qrFolder.isDir(dirpath)).catch(error => console.log(error));
            fileGallery[fileReference[dirpath]]?.refresh(dir);
          }
        } break;
        case "putContents": {
          let file = fileGallery[fileReference[message.filepath]];
          if (!file) return;
          rootFolder.dispatchEvent(new CustomEvent("filechanged", {
            detail: {
              fileElem: file,
