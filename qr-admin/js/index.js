document.addEventListener("DOMContentLoaded", () => {
  !function () {
    ...
    rootFolder.addEventListener("penddingtask", function (event) {
      var {task, index} = event.detail;
      if (!logoIcon.classList.contains("loading")) 
        logoIcon.classList.add("loading");
      if (!statusBox.style.display) statusBox.style.display = "block";
      statusBox.innerText = task;
      if (index === 1) {
        rootFolder.addEventListener("finishedtask", function () {
          removeLoading();
        }, {once: true});
      }
    });
    ...
    /*
    var resizery = bottomNav.querySelector("qr-resizery");
    var bottomHeight = sessionStorage.getItem("bottom-height") ?? localStorage.getItem("bottom-height");
    if (+bottomHeight) {
      document.body.style.setProperty("--bottom-height", +bottomHeight);
      if (!window.terminal) {
        window.terminal = document.createElement("iframe");
        terminal.src = "/terminal";
        bottomNav.append(terminal);
      }
    } else {
      document.body.classList.add("bottom-zero");
    }
    resizery.addEventListener("resizeStart", function () {
      resizery.startHeight = bottomNav.offsetHeight;
    });
    resizery.addEventListener("resize", function (event) {
      var newHeight = resizery.startHeight - event.detail, perCent;
      if (newHeight < 100) {
        perCent = 10000 / innerHeight;
        if (newHeight < 80) perCent = 0;
        else if (perCent > 80) perCent = 80;
      } else {
        perCent = newHeight * 100 / innerHeight;
        if (perCent > 80) perCent = 80;
      }
      document.body.style.setProperty("--bottom-height", perCent);
      localStorage.setItem("bottom-height",  perCent);
      sessionStorage.setItem("bottom-height",  perCent);
      window.dispatchEvent(new CustomEvent("resize"));
    });
    resizery.addEventListener("resizeEnd", function () {
      if (document.body.style.getPropertyValue("--bottom-height") === "0") {
        if (!document.body.classList.contains("bottom-zero")) document.body.classList.add("bottom-zero");
      } else if (document.body.classList.contains("bottom-zero")) {
        document.body.classList.remove("bottom-zero");
        if (!window.terminal) {
          window.terminal = document.createElement("iframe");
          terminal.src = "/terminal";
          bottomNav.append(terminal);
  ...
  !function () {
    ...
    var getEditor = qr.cacheCall((page, queue) => {
          page.getContents().then(response => {
            ...
            if (!response) error = "[Binary Data]";
            else ({error, contents} = response);
            if (error) {
              if (error === "[Binary Data]") {
                readOnly = "nocursor";
                contents = "[Binary Data] not supported for edit.";
              } else if (error === "-1") {
                readOnly = "nocursor";
                contents = "The file does not exists.";
              } else {
                readOnly = "nocursor";
                contents = error;
              }
    ...
    var updateEditor = (page, contents) => {
      ...
      if (editorElem) {
        ...
        if (contents === "[?-\ufffd->binary]") {
          readOnly = "nocursor";
          contents = "[Binary Data] not supported for edit.";
        } else if (contents === "[?-\ufffd->not-exist]") {
          readOnly = "nocursor";
          contents = "The file does not exists.";
        }
        if (readOnly) editor.setOption("readOnly", "nocursor");
    ...
    rootFolder.addEventListener("filechanged", event => {
      var {fileElem, contents} = event.detail,
          page = navBar.pageGallery[fileElem.fnum];
      updateEditor(page, contents);
    });
    ...
    refresh.addEventListener("click", () => {
      ...
      if (page && editor) {
        page.getContents().then(response => {
          var {error, contents} = response;
          if (error) {
            if (error === "[Binary Data]") contents = "[Binary Data] not supported for edit.";
            else if (error === "-1") contents = "The file does not exists.";
            else contents = error;
          }
          updateEditor(page, contents);
        });
